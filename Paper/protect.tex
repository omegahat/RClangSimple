...
\Cfunc{Rf_unprotect}, we have to arrange to get its argument.  We can
...
{
  R_AllocRoutineNames = c("Rf_allocVector", "NEW_NUMERIC", 
                          "NEW_INTEGER", "NEW_LOGICAL", "NEW_CHARACTER")
  numAllocs = 0  
  numProtectCalls = 0
...
\end{RCode}
%$
For the present, ignore the second expression in our visitor function,
the \Rfunc{if} expression.  We'll discuss this below.  However,
observe the code in the \Rexpr{if(unProtect)} block.  This becomes
active when we are within a call to \Cfunc{UNPROTECT}.  We check to
see if the current cursor is a \Rvar{CXCursor_IntegerLiteral} type or
a \Rvar{CXCursor_FirstExpr}.  The former indicates that we have a
literal integer value.  Unfortunately, there is no function in the
\libclang{} API for us to access that value directly. Instead, we use
the \Rfunc{getCursorTokens} function. This gives us the text around
the cursor, broken into tokens or atomic elements that make sense at
the \C/\Cpp-level.  The elements in the resulting character vector are
named with their token types and so we can extract the
\dquote{Literal} value.
...
\Cfunc{UNPROTECT} uses a variable, we can make a second pass over the
